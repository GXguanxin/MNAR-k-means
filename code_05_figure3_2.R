...
generate_data_different_MNAR_k4_p50 <- function(missmechanism,missproportion){
  if (missmechanism == "MNAR0"){
    if (missproportion == 0.1){
      data <- generate_data_20251216(n=100*4, p=50, k=4, 
                                     centers=rbind( c( 0,0 ,rep(0,48)) , c( 3,0 ,rep(0,48)), c( 0,3, rep(0,48)), c( 3,3 ,rep(0,48)) ), 
                                     variance_vec = c(rep(1,2),rep(1,48)),
                                     missmechanism="MNAR0",MNAR0lambda_star=45, nomissing=FALSE )
    }
...
experiment_different_MNAR_k4_p50 <- function(data,k=4,
                                             mustar=rbind( c( 0,0 ,rep(0,48)) , c( 3,0 ,rep(0,48)), c( 0,3, rep(0,48)), c( 3,3 ,rep(0,48)) ), 
  ...
  return(list( missdata_n=missdata_n,  missdata_proportion=missdata_proportion, selectedlambda=selectedlambda,
               cer_km=cer_km, cer_mice=cer_mice, cer_kpod=cer_kpod, cer_pkpod=cer_pkpod,
               mse_km=mse_km, mse_mice=mse_mice, mse_kpod=mse_kpod, mse_pkpod=mse_pkpod,
...
ExperimentResults_different_MNAR_k4_p50=list(
  allsettings=list(
    missmechanism_list=c("MNAR0","MNAR1","MNAR2"),
    MNAR0lambda_star_list=c(45, 5, 1.5),
...
for (missmechanism in c("MNAR0","MNAR1","MNAR2")){
  for (missproportion in c(0.1,0.3,0.5)){
    ...
    for (rept in 1:30){
      ...
      data <- generate_data_different_MNAR_k4_p50(missmechanism=missmechanism, missproportion=missproportion)
      res_once <- try( experiment_different_MNAR_k4_p50(data,k=4,
                                                        mustar=rbind( c( 0,0 ,rep(0,48)) , c( 3,0 ,rep(0,48)), c( 0,3, rep(0,48)), c( 3,3 ,rep(0,48)) ), 
    ...
    ExperimentResults_different_MNAR_k4_p50$allresults[[setting]] <- cbind(setting_mat, res_mat)
    setting <- setting + 1
...
ExperimentResults_different_MNAR_k4_p50$summarizedresults$cer <- as.data.frame(do.call(rbind, ExperimentResults_different_MNAR_k4_p50$allresults)) %>%
...
ExperimentResults_different_MNAR_k4_p50$summarizedresults$mse <- as.data.frame(do.call(rbind, ExperimentResults_different_MNAR_k4_p50$allresults)) %>%
...
ExperimentResults_different_MNAR_k4_p50$figures$cer <- ggplot(ExperimentResults_different_MNAR_k4_p50$summarizedresults$cer,
...
ExperimentResults_different_MNAR_k4_p50$figures$mse <- ggplot(ExperimentResults_different_MNAR_k4_p50$summarizedresults$mse,
...
ExperimentResults_different_MNAR_k4_p50$figures$cer
ExperimentResults_different_MNAR_k4_p50$figures$mse
